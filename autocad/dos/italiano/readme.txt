...
---------------------  questo driver deve giÖ essere installata sul PC.
		       AutoCAD deve essere stato eseguito almeno una volta
		       prima di installare il driver in modo da essere certi
		       che sia configurato correttamente e che sia stato 
		       creato il file ACAD.CFG (o ACADNT.CFG o ACAD14.CFG). 
...
1. - Il driver puï essere installato sul server ed usato da qualsiasi 
   computer ad esso collegato.

2. - Occorre disporre del permesso di scrittura per le directory 
   dell'unitÖ disco fisso del server. Se non si dispone di tale 
   permesso, la procedura di installazione deve essere eseguita 
   dall'amministratore del sistema o da un altro utente provvisto 
   delle apposite autorizzazioni.

3. - Non vi possono essere sessioni AutoCAD attive mentre si 
   installa il driver. Pertanto, durante la procedura di 
   installazione, si puï dover chiedere agli altri utenti di 
   chiudere le loro sessioni AutoCAD.


Errore di trasferimento file 
----------------------------
Se si riceve un messaggio che segnala un "errore generale di 
trasferimento file" durante l'installazione di questo driver, 
accertarsi di disporre del permesso di scrittura per la directory 
e tutte le sottodirectory selezionate per l'installazione.


Se si sta aggiornando il driver esistente a v4.0
------------------------------------------------
Se AutoCAD ä giÖ configurato per usare una versione pió 
vecchia di questo driver (v3.3 o precedente), si noti che 
occorrerÖ ripetere la procedura di configurazione del plotter 
in AutoCAD in modo che possa utilizzare la versione 4.0 del driver. 
Si tenga presente, inoltre, che con l'installazione di questa 
nuova versione verrÖ rimossa la versione precedente del driver. 

Per ulteriori informazioni relative a sistemi specifici, vedere 
pió avanti.


Annullamento di un lavoro
-------------------------
Non ä possibile annullare un lavoro con la funzione di controllo 
dello stato se il lavoro fa parte di un annidamento di lavori. 
Tuttavia, l'annullamento avrÖ effetto sul successivo annidamento 
se questo contiene delle pagine del lavoro annullato. 


Sessioni AutoCAD multiple
-------------------------
Sebbene AutoCAD r13 e r14 consentano di avere attive pió sessioni
contemporaneamente, il driver puï operare solo con una di esse.


Formato minimo dei supporti
---------------------------
Quando si specificano (in PLOT, HPMPLOT o HPRENDER) delle 
dimensioni utente inferiori a quelle corrispondenti ad un 
formato A4 e si utilizza un supporto in rotoli, il disegno 
viene stampato con le dimensioni corrette, ma ä possibile 
che il supporto venga tagliato come per il formato A4.
...
1. - Accertarsi di aver installato l'ultima versione 
    (C4 o successiva) di AutoCAD r12 per Windows. Autodesk 
    fornisce gratuitamente un aggiornamento a questa versione 
    per tutti gli utenti di AutoCAD per Windows.
...
Questa versione richiede una considerevole quantitÖ di memoria. Se
...
che questo ä solo per la configurazione base.
...
o inadeguato, ä possibile correggere manualmente il file acad14.cfg
facendo quanto segue:

1. - Accertarsi di essere usciti da AutoCAD e che AutoCAD non sia attivo.

2. - Aprire il file acad14.cfg con un qualsiasi editor di testo. Il file 
     acad14.cfg ä un file ASCII e quindi puï essere aperto con quasi tutti 
     gli editor di testo.
...
3. - Sostituire ovunque la stringa:
    ...
    Cosç facendo AutoCAD userÖ il nuovo driver con i dispositivi
    associati.
...
1. - Se le stampanti sono state configurate per utilizzare una determinata
    porta LPTx e si tenta di stampare da AutoCAD utilizzando quella stessa 
    porta, comparirÖ un messaggio di errore indicante che AutoCAD non ä in 
    grado di impostare la porta. Ciï dipende dal fatto che AutoCAD tenta di 
    accedere direttamente all'hardware. Per evitare questo tipo di problemi, 
    non vi devono essere stampanti configurate per l'uso della porta LPTx che 
    si intende utilizzare da AutoCAD.
...
server?
    ...
    b) Configurare il plotter in AutoCAD come di consueto. Quando viene 
       richiesto di specificare il numero di una porta COM/LPT, immettere 
       un punto (.) e non indicare cosç alcuna porta.
...
3. - Se il programma di installazione non riesce a rilevare un'installazione
    ...
    Se si sta usando Windows NT 4.0 e si ä installato AutoCAD sotto un
    conto con privilegi di amministratore, ä necessario installare anche
    ...
    problema anche immettendo l'esatta ubicazione della directory in cui ä
...
localizzata del driver AutoCAD ä supportata nei Paesi asiatici.
